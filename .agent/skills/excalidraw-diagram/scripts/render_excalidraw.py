...
Usage:
    cd .agent/skills/excalidraw-diagram
    uv run python scripts/render_excalidraw.py <path-to-file.excalidraw> [--output path.png] [--scale 2] [--width 1920]
...
def render(
    ...
    try:
        ...
        print("Run: cd .agent/skills/excalidraw-diagram && uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    # Template path (assets/ directory, one level up from scripts/)
    template_path = Path(__file__).parent.parent / "assets" / "render_template.html"
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print("Run: cd .agent/skills/excalidraw-diagram && uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
