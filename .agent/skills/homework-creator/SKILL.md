...
├── Week_[N]_Reading_L1.docx        ← Reading text — Level 1 (12–13 yrs)
├── Week_[N]_Reading_L2.docx        ← Reading text — Level 2 (10–11 yrs)
├── Week_[N]_Reading_L3.docx        ← Reading text — Level 3 (8–9 yrs)
├── Week_[N]_Questions_L1.docx      ← L1 comprehension + Year 5 maths (Forms-ready)
├── Week_[N]_Questions_L2.docx      ← L2 comprehension + Year 5 maths (Forms-ready)
├── Week_[N]_Questions_L3.docx      ← L3 comprehension + Year 3/4 maths (Forms-ready)
├── Week_[N]_Print_L1.docx          ← Two-page student printable for L1
├── Week_[N]_Print_L2.docx          ← Two-page student printable for L2
├── Week_[N]_Print_L3.docx          ← Two-page student printable for L3
...
**Maths pairing rule:**
- L1 questions document → Year 5 maths
- L2 questions document → Year 5 maths
- L3 questions document → Year 3/4 maths
...
| Level | Target Age | Word Count  | Avg Sentence Length | Vocabulary              |
|-------|-----------|-------------|---------------------|-------------------------|
| L1    | 12–13 yrs | 350–450 wds | 18–25 words         | Subject-specific terms  |
| L2    | 10–11 yrs | 250–350 wds | 12–18 words         | Moderate complexity     |
| L3    | 8–9 yrs   | 150–250 wds | 8–12 words          | Simple, decodable words |

### Validation Protocol (using `text-analysis` skill)

After writing each text:

1. Save the plain text to a temporary file: `Homework/Week_[N]/tmp_L[N].txt`
2. Run the readability check:
   ```bash
   python .agent/skills/text-analysis/scripts/analyse_text.py Homework/Week_[N]/tmp_L[N].txt --level L[N]
   ```
...
- Subtitle: student's reading level is NOT labelled on the student document
...
Generate **three separate sets** of 15 comprehension questions, one for each reading level. Every question in a set must be answerable solely from the text at that level — do not draw on information that appears only in a higher-level version of the text.

| Set | Paired text | Question style |
|-----|------------|----------------|
| L1  | Week_[N]_Reading_L1.docx | Inference-heavy; students must read between the lines |
| L2  | Week_[N]_Reading_L2.docx | Mix of literal and inferential; accessible for 10–11 yr olds |
| L3  | Week_[N]_Reading_L3.docx | Literal comprehension; simple question language |

**Critical rule:** Before writing a question, verify that its answer can be found in or logically inferred from the specific text level it is paired with. If a concept appears in L1 but not in L3 (e.g., "the Coriolis effect", "wind shear"), it must not appear in the L3 question set.
...
Compile three DOCX files using the `microsoft-forms-assessment` and `docx` skills — one per reading level.

**Week_[N]_Questions_L1.docx**
- Questions 1–15: L1 comprehension questions (based on L1 reading text)
- Questions 16–30: Year 5 maths questions
- Numbered continuously from 1 to 30

**Week_[N]_Questions_L2.docx**
- Questions 1–15: L2 comprehension questions (based on L2 reading text)
- Questions 16–30: Year 5 maths questions
- Numbered continuously from 1 to 30

**Week_[N]_Questions_L3.docx**
- Questions 1–15: L3 comprehension questions (based on L3 reading text only)
...
After all content is generated and verified, execute the `homework-print` skill to generate the three two-page student printable files (`Week_[N]_Print_L1.docx`, `Week_[N]_Print_L2.docx`, `Week_[N]_Print_L3.docx`). Follow the `homework-print` skill instructions exactly, which involves using a Node.js script to create the clean, formatted two-page layouts without answers.
...
- [ ] Three reading DOCX files created and validated against reading level targets
- [ ] Three questions DOCX files created in correct Microsoft Forms format (L1, L2, L3)
- [ ] Three print DOCX files generated and verified (L1, L2, L3) via homework-print skill
- [ ] All comprehension questions verified as answerable from their paired reading text
- [ ] L1 and L2 questions documents use Year 5 maths
- [ ] L3 questions document uses Year 3/4 maths
