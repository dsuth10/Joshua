...
The aesthetic is a hybrid of **Modern Corporate** and **Neo-Brutalism**. It utilizes structured grids, high-contrast typography, and functional "glassmorphism" for utility layers (like toolbars), while employing heavy "hard shadows" and thick borders for interactive elements. This approach, "Visual Weight as Feedback," ensures that every interactable component feels tactile and physically present, reducing cognitive load for students by clearly signaling clickability and state changes.

## Colors

The color strategy uses a deep Navy (`primary`) to establish authority and trust, paired with a vibrant Orange (`secondary`) used exclusively for calls-to-action, highlights, and interactive accents.
...
- **Thematic Variants**: Soft blue and soft orange tints are used for background categorization (e.g., differentiating 12-hour and 24-hour time formats).
...
The layout uses a **Fixed Grid** philosophy optimized for landscape presentation screens (16:9 aspect ratio). 
...
- **Grid Systems**: Interactive tasks (sorting, matching, quizzes) typically utilize a `repeat(2, 1fr)` or `repeat(3, 1fr)` grid with consistent 24px gaps.
- **Responsive Reflow**: On smaller breakpoints, 2-column grids collapse into a single vertical column. Content margins reduce from 70px to 24px to maximize screen real estate.
...
- **Visuals**: Soft grey background with a thick (6px) vertical accent border on the left side. The color of the accent border indicates the type of content (e.g., Orange for "Remember," Blue for "Scenario").

### Presentation Toolbar
- **Style**: Floating capsule, blurred navy background.
- **Icons**: 24px SVGs in `text-muted` color, shifting to `secondary` (Orange) when active.