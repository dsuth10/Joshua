// Copied from anteater-website/script.js with console message updated
...
const counterObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            const counters = entry.target.querySelectorAll('.stat-number, .diet-number');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (dietSection) counterObserver.observe(dietSection);
});
...
style.textContent = `
    ...
    body:not(.loaded)::before { content: ''; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(135deg, #2c5530, #4a90e2); z-index: 9999; display: flex; align-items: center; justify-content: center; }
...
window.addEventListener('scroll', () => {
    if (window.pageYOffset > 300) { backToTop.style.opacity = '1'; backToTop.style.visibility = 'visible'; }
    else { backToTop.style.opacity = '0'; backToTop.style.visibility = 'hidden'; }
});
...
console.log('Amazon Mining Threat website loaded successfully!');
