...
function title(group, subtitle) {
  return [
    p("TERM 3 - WEEK 3 HOMEWORK", { align: AlignmentType.CENTER, bold: true, size: 25, color: "173F5F", spacing: { after: 20, line: 250 } }),
...
function compactQ(q, n, width, maths = false) {
  const tab = Math.floor(width / 2);
  return [
    new Paragraph({
      keepNext: true,
      spacing: { before: maths ? 20 : 30, after: 0, line: 210 },
      children: [new TextRun({ text: `${n}. ${q.q}`, bold: true, size: 20, font: "Arial" })]
    }),
    new Paragraph({
      keepNext: true,
...
function printDoc(group) {
  ...
  const children = [
    ...
    ...t.paragraphs.map(x => p(x, { size: 24, spacing: { after: 70, line: 312 } })),
    ...
    p("MATHEMATICS - METRIC CONVERSION WORD PROBLEMS", { bold: true, size: 20, color: "173F5F", spacing: { before: 45, after: 10, line: 210 } }),
    new Table({
  ...
  return new Document({
    ...
    sections: [{
      ...
      headers: { default: new Header({ children: [p("Homework | Term 3 | Week 3", { align: AlignmentType.RIGHT, size: 15, color: "6B7280" })] }) },
...
(async () => {
  for (const group of ["Red", "Blue", "Green"]) {
    fs.writeFileSync(path.join(OUT, `Week_3_Print_${group}.docx`), await Packer.toBuffer(printDoc(group)));
  }
  console.log("Created 3 student print documents.");
