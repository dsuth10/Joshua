...
function header(group, kind) {
  return new Header({ children: [new Paragraph({
    alignment: AlignmentType.RIGHT,
    spacing: { after: 0 },
    children: [new TextRun({ text: `Term 3 | Week 3 | ${kind}`, size: 17, color: "6B7280", font: "Arial" })]
  })] });
}
function footer() {
...
function baseDoc(children, group, kind) {
  return new Document({
    styles,
    sections: [{
      properties: { page: PAGE },
      headers: { default: header(group, kind) },
...
function readingDoc(group) {
  const t = texts[group];
  const children = [
    ...titleBlock("Week 3 Homework - Information Text", t.title),
    ...t.paragraphs.map(p => new Paragraph({
      spacing: { after: 150, line: 312 },
      children: [new TextRun({ text: p, font: "Arial", size: 24 })]
    }))
  ];
  return baseDoc(children, group, "Reading");
}
function qParas(q, n) {
  return [
    new Paragraph({ keepNext: true, spacing: { before: 140, after: 60 }, children: [new TextRun({ text: `${n}. ${q.q}`, bold: true, size: 22 })] }),
    ...["a", "b", "c", "d"].map((k, i) => new Paragraph({ spacing: { after: 30 }, indent: { left: 360 }, children: [new TextRun({ text: `${"ABCD"[i]}. ${q[k]}`, size: 21 })] })),
    new Paragraph({ spacing: { after: 20 }, children: [new TextRun({ text: `ANSWER: ${q.ans}`, bold: true, size: 20, color: "173F5F" })] }),
...
function questionsDoc(group) {
  ...
  const children = [
    ...titleBlock(`Week 3 Questions - ${group}`, "Microsoft Forms import document"),
    new Paragraph({ style: "Heading1", children: [new TextRun("Reading comprehension")] }),
    ...comp[group].flatMap((q, i) => qParas(q, i + 1)),
    new Paragraph({ style: "Heading1", pageBreakBefore: true, children: [new TextRun("Mathematics - Metric conversion word problems")] }),
    ...maths.flatMap((q, i) => qParas(q, i + 16))
  ];
  return baseDoc(children, group, "Questions");
}
...
(async () => {
  for (const group of ["Red", "Blue", "Green"]) {
    await write(`Week_3_Reading_${group}.docx`, readingDoc(group));
    await write(`Week_3_Questions_${group}.docx`, questionsDoc(group));
  }
  console.log("Created 6 reading and Forms-ready question documents.");
