...
function header(kind) {
  return new Header({ children: [new Paragraph({
    ...
    spacing: { after: 0 },
    children: [new TextRun({ text: `Term 3 | Week 9 | ${kind}`, size: 17, color: "6B7280", font: "Arial" })]
  })] });
}
function footer() {
...
function readingDoc(group) {
  const t = texts[group];
  return baseDoc([
    ...titleBlock("Week 9 Homework — Narrative Text", t.title),
...
function questionsDoc(group) {
  ...
  const mathHeading = group === "Green"
    ? "Mathematics — Multi-Step Financial Problem Solving (Foundational)"
    : "Mathematics — Multi-Step Financial Problem Solving & Estimation";
  return baseDoc([
    ...titleBlock(`Week 9 Questions — ${group}`, "Microsoft Forms import document"),
...
(async () => {
  for (const group of ["Red", "Blue", "Green"]) {
    await write(`Week_9_Reading_${group}.docx`, readingDoc(group));
    await write(`Week_9_Questions_${group}.docx`, questionsDoc(group));
  }
  console.log("Created 6 reading and Forms-ready question documents for Week 9.");
