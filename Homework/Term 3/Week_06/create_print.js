...
function title(group, subtitle) {
  return [
    p("TERM 3 - WEEK 6 HOMEWORK", { align: AlignmentType.CENTER, bold: true, size: 25, color: "173F5F", spacing: { after: 20, line: 250 } }),
...
function printDoc(group) {
  ...
  return new Document({
    ...
    sections: [{
      ...
      headers: { default: new Header({ children: [p("Homework | Term 3 | Week 6", { align: AlignmentType.RIGHT, size: 15, color: "6B7280" })] }) },
...
(async () => {
  for (const group of ["Red", "Blue", "Green"]) {
    fs.writeFileSync(path.join(OUT, `Week_6_Print_${group}.docx`), await Packer.toBuffer(printDoc(group)));
  }
  console.log("Created 3 student print documents for Week 6.");
