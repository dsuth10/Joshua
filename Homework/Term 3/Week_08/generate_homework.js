...
function header(kind) {
  return new Header({ children: [new Paragraph({
    ...
    spacing: { after: 0 },
    children: [new TextRun({ text: `Term 3 | Week 8 | ${kind}`, size: 17, color: "6B7280", font: "Arial" })]
  })] });
}
function footer() {
...
function readingDoc(group) {
  const t = texts[group];
  return baseDoc([
    ...titleBlock("Week 8 Homework - Persuasive Text", t.title),
...
function questionsDoc(group) {
  ...
  return baseDoc([
    ...titleBlock(`Week 8 Questions - ${group}`, "Microsoft Forms import document"),
...
(async () => {
  for (const group of ["Red", "Blue", "Green"]) {
    await write(`Week_8_Reading_${group}.docx`, readingDoc(group));
    await write(`Week_8_Questions_${group}.docx`, questionsDoc(group));
  }
  console.log("Created 6 reading and Forms-ready question documents for Week 8.");
