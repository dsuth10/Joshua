...
function header(kind) {
  return new Header({ children: [new Paragraph({
    ...
    spacing: { after: 0 },
    children: [new TextRun({ text: `Term 3 | Week 4 | ${kind}`, size: 17, color: "6B7280", font: "Arial" })]
  })] });
}
function footer() {
...
function readingDoc(group) {
  const t = texts[group];
  return baseDoc([
    ...titleBlock("Week 4 Homework - Information Text", t.title),
...
function questionsDoc(group) {
  ...
  return baseDoc([
    ...titleBlock(`Week 4 Questions - ${group}`, "Microsoft Forms import document"),
    new Paragraph({ style: "Heading1", children: [new TextRun("Reading comprehension")] }),
    ...comp[group].flatMap((q, i) => qParas(q, i + 1)),
    new Paragraph({ style: "Heading1", pageBreakBefore: true, children: [new TextRun("Mathematics - Equivalent fractions, then addition and subtraction")] }),
...
(async () => {
  for (const group of ["Red", "Blue", "Green"]) {
    await write(`Week_4_Reading_${group}.docx`, readingDoc(group));
    await write(`Week_4_Questions_${group}.docx`, questionsDoc(group));
  }
