...
function title(group, subtitle) {
  return [
    p("TERM 3 - WEEK 4 HOMEWORK", { align: AlignmentType.CENTER, bold: true, size: 25, color: "173F5F", spacing: { after: 20, line: 250 } }),
...
function printDoc(group) {
  ...
  return new Document({
    ...
    sections: [{
      ...
      headers: { default: new Header({ children: [p("Homework | Term 3 | Week 4", { align: AlignmentType.RIGHT, size: 15, color: "6B7280" })] }) },
      ...
      children: [
        ...
        p("MATHEMATICS - EQUIVALENT FRACTIONS, THEN ADDITION AND SUBTRACTION", { bold: true, size: 20, color: "173F5F", spacing: { before: 45, after: 10, line: 210 } }),
        new Table({
...
(async () => {
  for (const group of ["Red", "Blue", "Green"]) {
    fs.writeFileSync(path.join(OUT, `Week_4_Print_${group}.docx`), await Packer.toBuffer(printDoc(group)));
  }
