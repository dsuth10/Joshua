...
**Prerequisites (complete):** Phase 0 spikes ✅ · Phase 1 engine core ✅ · Phase 2 big six ✅ · Phase 3 practice ✅ (G3) · **Phase 4 assessments ✅ (G4 PASS 2026-06-13)**

**Gate G4:** all four assessments completable start-to-finish with **identical scoring**; bonus points still write to `joshua_math_profile` with the same keys and scaling as pre-migration. **PASS (2026-06-13)** — `g4-all-assessments-audit.mjs` + `g4-golden-path.mjs` (evidence: `../g4-golden-path-evidence/`).

**Session record:** [SESSION-LOG-2026-06-13.md](../SESSION-LOG-2026-06-13.md) (includes 4c Slice 4 number-line + golden-path close-out).
...
| 4b | `year5.js` | [02-year5-assessment.md](02-year5-assessment.md) | ~1,018 | accordion + dispatch grid | ✅ **G4b static PASS (2026-06-13)** — line-count target open |
| 4c | `year4.js` | [03-year4-assessment.md](03-year4-assessment.md) | ~996 | — | ✅ **G4c static PASS (2026-06-13)** |
| 4d | `year3.js` | [04-year3-assessment.md](04-year3-assessment.md) | ~1,161 (−280) | none — all on engine | ✅ **G4d static PASS (2026-06-13)** |
...
```bash
node scripts/g4-all-assessments-audit.mjs   # combined static + smoke
node scripts/g4-golden-path.mjs             # profile golden-path (G4 sign-off)
node scripts/g4-y3-assessment-audit.mjs     # per-year (also run via combined)
...
- [x] Complete one full mission per year; profile bonus verified — `node scripts/g4-golden-path.mjs` (2026-06-13); Y3 SS2 fraction pin: confirm manual drag to 3/4 for 30/30
...
| 4b Y5 dispatch + decimal | **M** | ✅ Done — `plot-waypoints` + `accordion-decimal`; G4b static PASS |
| 4c Y4 wiring | **S–M** | ✅ Stage 3 static PASS — `alpha-grid` dual-select + `symmetry-painter` |
| 4d Y3 full sweep | **L** | ✅ Static PASS — ~280 lines SVG deleted; golden-path PASS (29/30 auto) |
| G4 audit scripts + QA | **S** per file | ✅ Static + golden-path PASS — `g4-golden-path.mjs` evidence in `g4-golden-path-evidence/` |

**Coordinate-plotter family consolidation (2026-06-13):** Y5 `makeAssessmentGridSvg` deleted → `plot-waypoints`; Y3 `initDeliveryGridMap` deleted → `path-rover`. Shared grid animation pattern now lives in `mcs-widgets-space.js`.

**Recommended next step:** **Phase 5** — Prep–Year 2 build-out (see 07-Roadmap §Phase 5).
