# Ginger Juice Voice Bake-off Scorecard

**Gate:** G1  
**Approver:**  
**Listening date:**  
**Selected candidate:**  

All candidates narrate the same three source-faithful passages. Listen with the same
headphones or speakers and avoid judging from only the opening passage.

## Scoring

Score each criterion from 1–5. The weighted score is calculated as:

`score / 5 × weight`

| Candidate | Naturalness 25% | Accuracy 20% | Emotion 20% | Consistency 15% | Pronunciation 10% | Speed/stability 10% | Weighted total |
|---|---:|---:|---:|---:|---:|---:|---:|
| Kokoro — bf_emma |  |  |  |  |  |  |  |
| Qwen CustomVoice — Serena |  |  |  |  |  |  |  |
| Qwen designed voice → Base clone |  |  |  |  |  |  |  |
| Chatterbox Multilingual V3 — default |  |  |  |  |  |  |  |

## Mandatory integrity review

For every candidate and passage:

- [ ] No omitted clause
- [ ] No repeated clause
- [ ] No invented words
- [ ] No clipping or corrupted audio
- [ ] Stable voice identity across all three passages
- [ ] No comic or caricatured “ape voice”

Any unchecked item is disqualifying.

## Pronunciation decisions

Listen to the baseline and override samples before choosing a form.

| Term | Baseline preferred | Override preferred | Approved spoken form | Notes |
|---|:---:|:---:|---|---|
| Ibu |  |  |  |  |
| macaques |  |  |  |  |
| gibbons |  |  |  |  |
| cicadas |  |  |  |  |
| katydids |  |  |  |  |
| durians |  |  |  |  |
| papayas |  |  |  |  |
| rambutans |  |  |  |  |

## Candidate notes

### Kokoro — bf_emma


### Qwen CustomVoice — Serena


### Qwen designed voice → Base clone


### Chatterbox Multilingual V3 — default


## Gate G1 decision

- [ ] I approve the selected candidate as the production voice.
- [ ] I approve the pronunciation forms recorded above.
- [ ] I approve progression to full-chapter rendering.

**Approval notes:**

